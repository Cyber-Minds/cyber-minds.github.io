...
const iamRequiredPermissions = {
  's3:getobject':
    'arn:aws:s3:::cm-backup-data-123456789012/prod/daily/*',
  's3:listbucket': 'arn:aws:s3:::cm-backup-data-123456789012',
...
const iamSetupScript = [
  "if [ ! -f /workspace/policy.json ]; then",
...
const iamCheckScript = [
  ...
  'import json, sys',
  `required_permissions = ${JSON.stringify(iamRequiredPermissions)}`,
  'MAX_BYTES = 50_000',
  'try:',
  ...
  '        print(f"FAIL: statement {i+1} must be an object")',
  '        sys.exit(1)',
  ...
  '        print(f"FAIL: statement {i+1} must use explicit Action and Resource values")',
  '        sys.exit(1)',
  ...
  '        if principal == "*" or aws_principal == "*" or (isinstance(aws_principal, list) and "*" in aws_principal):',
  ...
  '        print(f"FAIL: statement {i+1} must contain explicit Action and Resource values")',
  '        sys.exit(1)',
  '    for a in actions:',
  '        if not isinstance(a, str) or a.lower() not in required_permissions:',
  '            print(f"FAIL: statement {i+1} contains an action outside the four required permissions")',
  '            sys.exit(1)',
  '    for r in resources:',
  '        if not isinstance(r, str) or r.lower() not in required_permissions.values():',
  '            print(f"FAIL: statement {i+1} contains a resource outside the required ARNs")',
  '            sys.exit(1)',
  '    for a in actions:',
  '        for r in resources:',
  '            allowed_pairs.add((a.lower(), r.lower()))',
  'required_pairs = set((action, resource) for action, resource in required_permissions.items())',
  'if allowed_pairs != required_pairs:',
  '    print("FAIL: permissions must match the four required action/ARN pairs exactly")',
  '    sys.exit(1)',
...
const phishingCheckScript = [
  ...
  '    sys.exit(1)',
  'if not re.search(r"\\bcorporate-alerts\\.example\\.com\\b", content, re.I):',
